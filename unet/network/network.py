...
class UNet(paddle.nn.Layer):
	...
	def _build_network(self, inputs, training):
		...
		inputs = paddle.cast(inputs, paddle.float32)
		
		# print(inputs.shape)
		inputs = convolution(inputs.shape[-1], self.first_output_filters, 3, 1, False)(inputs)

		# encoding_block_1
		with paddle.static.name_scope('encoding_block_1'):
		...
		for i in range(2, self.depth+1):
			...
			with paddle.static.name_scope('encoding_block_%d' % i):
		...
		with paddle.static.name_scope('bottom_block'):
		...
		for j in range(self.depth-1, 0, -1):
			with paddle.static.name_scope('decoding_block_%d' % j):
				...
				if self.skip_method == 'add':
					inputs = paddle.add(inputs, skip_inputs[j-1])
				elif self.skip_method == 'concat':
					inputs = paddle.concat([inputs, skip_inputs[j-1]], axis=-1)
		...
		inputs = convolution(inputs.shape[-1], 1, self.out_kernel_size, 1, self.out_kernel_bias)(inputs)
